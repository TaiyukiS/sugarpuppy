<template>
  <div class="flex items-center">
    <div
      ...
      v-bind:style="{
        backgroundImage: 'url('+user.url_capa+')'
      }"
    >
      <img
        alt="Foto do Perfil"
...
export default {
  ...
  methods: {
    ...
    buscarPosts (done) {
      ...
      PostService.get({ id_usuario: this.$route.query.id })
        .then(newPosts => {
          this.indexPagina = 1
          this.msgFim = false
    ...
    async buscarUsuario () {
      this.user = await UserService.get({ id: this.$route.query.id })
    ...
    buscarMaisPosts () {
      ...
      this.indexPagina++
      PostService.get({ pagina: this.indexPagina, user: this.$route.query.id })
        .then(newPosts => {
